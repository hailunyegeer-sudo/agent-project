...
from apischema.json_schema import deserialization_schema as _apischema_schema # type: ignore
...
def _build_schema_from_yaml(name: str, config: dict) -> dict:
...
def _build_schema_from_decorator(name: str) -> dict:
    module = importlib.import_module(f"skills.{name}")
    func = getattr(module, name)
    meta = getattr(func, "__tool_meta__", None)
    if meta is None:
        raise ValueError(f"skill {name} is missing @tool decorator")
    if "description" not in meta or "parameters" not in meta or "returns" not in meta:
        raise ValueError(f"tool {name} has incomplete @tool metadata")
    ...
    for param_name, param in sig.parameters.items():
        if param_name in _INFRA_PARAMS:
            continue
        py_type = hints.get(param_name, str)
        param_desc = meta["parameters"].get(param_name)
        if param_desc is None:
            raise ValueError(f"tool {name} missing parameter description for {param_name}")
        prop = _py_type_to_param_schema(py_type)
        if not prop:
            raise ValueError(f"unsupported type for {name}.{param_name}: {py_type}")
...
def get_tools_schema(
    ...
    for name in tool_names:
        try:
            entry = _build_schema_from_decorator(name)
        except (ImportError, AttributeError, ValueError):
            entry = _build_schema_from_yaml(name, config)
        schema.append(entry)
...
_INFRA_PARAMS = {"data_root", "output_dir"}


def _py_type_to_param_schema(py_type: type) -> dict:
    raw = _apischema_schema(py_type)
    # Unwrap top-level anyOf [..., null] → the concrete alternative
    if "anyOf" in raw and "type" not in raw:
        concretes = [alt for alt in raw["anyOf"] if alt.get("type") != "null"]
        if len(concretes) == 1:
            raw = concretes[0]
    if isinstance(raw.get("type"), list):
        types = [t for t in raw["type"] if t != "null"]
        if types:
            raw["type"] = types[0]
    result: dict = {}
    if "type" in raw:
...
def execute_tool_calls(
            ...
            if name not in allowed_tools or name not in config["tools"]:
                ...
                try:
                    ...
                    if isinstance(exc, (ImportError, AttributeError)):
                        exc = RuntimeError(f"cannot load configured tool {name}: {exc}")
    ...
    if outdir:
        ...
        log_records = _read_jsonl(output_dir / "tool_call_log.jsonl")
        write_json(compute_statistics(log_records), output_dir / "tool_call_stats.json")
    return tool_messages


def _read_jsonl(path: str | Path) -> list[dict]:
    """Read a JSONL file into a list of dicts. Returns empty list if file does not exist."""
    p = Path(path)
    if not p.exists():
        return []
    records = []
...
def compute_statistics(log_records: list[dict]) -> dict:
    """Compute aggregated statistics from tool call log records."""
    if not log_records:
        return {
            "total_calls": 0,
            "success_count": 0,
    ...
    for r in log_records:
        ...
        if name not in by_tool:
            by_tool[name] = {"total": 0, "success": 0, "error": 0, "latencies": []}
        by_tool[name]["total"] += 1
        if r["status"] == "success":
            by_tool[name]["success"] += 1
        else:
            by_tool[name]["error"] += 1
        if r["latency_ms"] is not None:
            by_tool[name]["latencies"].append(r["latency_ms"])

    per_tool = {}
    for name, stats in sorted(by_tool.items()):
        tool_avg = (
            round(sum(stats["latencies"]) / len(stats["latencies"]), 3)
            if stats["latencies"]
            else 0.0
        )
        per_tool[name] = {
            ...
            "error": stats["error"],
            "failure_rate": round(stats["error"] / stats["total"], 4),
            "avg_latency_ms": tool_avg,
        }
    ...
    return {
        ...
        "failure_rate": round(error_count / total, 4),
